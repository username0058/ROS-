...
def callback(data):
    ...
    img = white_balance(cv_img,mode=2)
    hsv_image = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)

    low_range = np.array([160, 60, 100])
    high_range = np.array([180, 220, 230])
    ...
    #cv2.waitKey(0)
    image_pub_again=rospy.Publisher('image_color_recognition', Image, queue_size = 5) 
    image_pub_again.publish(bridge.cv2_to_imgmsg(img, "bgr8"))
    rospy.loginfo("Great!Running img_sub node.The image is publishing again.")
...
if __name__ == '__main__':
    try:
        ...
        print ("Oh,NO!Something is wrong!Shutting down img_sub node.")
        cv2.destroyAllWindows()
